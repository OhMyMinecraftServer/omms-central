...
import net.zhuruoling.omms.central.network.session.request.RequestManager.getRequestHandler
...
class SessionServer(private val session: Session, private var permissions: List<Permission>) : Thread() {
    ...
    override fun run() {
        ...
        sessionContext = SessionContext(this, fuseEncryptedSocket, session, permissions)
        try {
            while (true) {
                try {
                    ...
                    if (permission != null && !permissions.contains(permission)) {
                        sendResponseAsync(Response().withResponseCode(Result.PERMISSION_DENIED))
                        continue
                    }
                    ...
                    try {
                        ...
                        if (response == null) {
                            ...
                            fuseEncryptedSocket.sendResponse(Response().withResponseCode(Result.DISCONNECT))
                            session.socket.close()
                            break
                        }
                    } catch (t: Throwable) {
                        t.printStackTrace()
                        response = Response().withResponseCode(Result.FAIL).withContentPair("error", t.toString())
                    }
                    ...
                    sendResponseAsync(Response().withResponseCode(Result.RATE_LIMIT_EXCEEDED))
