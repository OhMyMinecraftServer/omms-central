                ...
                if (Objects.requireNonNull(request).request == "PING") {
                    if (request.version != Util.PROTOCOL_VERSION) {
                        encryptedConnector.send(
                            Response.serialize(Response().withResponseCode(Result.VERSION_NOT_MATCH))
                        )
                        break
                    }
                    ...
                    if (isCodeExist) {
                        ...
                        val session =
                            SessionServer(Session(socket, randomKey.toByteArray(StandardCharsets.UTF_8)), permissions!!)
                        session.start()
                        break
                    } else {
                        encryptedConnector.send(
