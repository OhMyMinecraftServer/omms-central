...
import net.zhuruoling.omms.central.permission.player.Group
import net.zhuruoling.omms.central.permission.player.PlayerData
    ...
    fun getAllPlayer(): List<PlayerData> {
        ...
        if (result.first) {
            ...
            while (resultSet.next()) {
                ...
                val playerData = PlayerData(name, groups.split(", "), servers.split(", "))
                list.add(playerData)
            }
        }
        return list
    ...
    fun getAllGroup(): List<Group> {
        ...
        if (result.first) {
            ...
            while (resultSet.next()) {
                val servers = resultSet.getString("servers")
                val name = resultSet.getString("name")
                val group = Group(name, servers.split(", "))
                list.add(group)
            }
        }
        return list
